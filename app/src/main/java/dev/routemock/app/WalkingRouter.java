...
final class WalkingRouter {
    static final String USER_AGENT = "RouteMock/0.2 (Android route simulation prototype)";
    private static long lastRequestNanos;
    private WalkingRouter() {}

    static List<GeoPoint> plan(List<GeoPoint> waypoints, PlaybackMode mode) throws IOException {
        URL url = requestUrl(waypoints, mode);
        ...
        try {
            ...
            return parseRoute(new String(data, StandardCharsets.UTF_8), mode);
    ...
    static URL requestUrl(List<GeoPoint> waypoints, PlaybackMode mode) throws IOException {
        Objects.requireNonNull(mode, "mode");
        if (waypoints.size() < 2 || waypoints.size() > 8) throw new IOException("請先選擇 2–8 個途經點");
        List<GeoPoint> requestPoints = new ArrayList<>(waypoints);
        if (mode == PlaybackMode.LOOP && !waypoints.get(0).equals(waypoints.get(waypoints.size() - 1)))
            requestPoints.add(waypoints.get(0));
        StringBuilder coordinates = new StringBuilder();
        for (GeoPoint p : requestPoints) {
            if (coordinates.length() > 0) coordinates.append(';');
            coordinates.append(String.format(Locale.US, "%.7f,%.7f", p.longitude(), p.latitude()));
        }
        return new URL("https://routing.openstreetmap.de/routed-foot/route/v1/foot/" + coordinates
                + "?overview=full&geometries=geojson&steps=false&radiuses=" + "100;".repeat(requestPoints.size() - 1) + "100");
    }
    static List<GeoPoint> parseRoute(String text, PlaybackMode mode) throws IOException {
        Objects.requireNonNull(mode, "mode");
        if (text.getBytes(StandardCharsets.UTF_8).length > 2 * 1024 * 1024) throw new IOException("路線太長，請縮短範圍");
        try {
            ...
            if (mode == PlaybackMode.LOOP && !points.get(0).equals(points.get(points.size() - 1)))
                throw new IOException("規劃服務未回傳閉合步行路線，請調整選點後重試");
